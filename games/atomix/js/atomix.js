...
(function () {
    ...
    function create_box() {
        return gA.new_img('box', 'box', -1, 'box');
    }

    function show_box() {
        var row = gCurrent.row, col = gCurrent.col;
        xMoveTo(gBox.box, gA.xpos(col), gA.ypos(row));
        gBox.visible = true;
        gBox.row = row;
        gBox.col = col;
    }
    ...
    function toggle_current() {
        ...
        if (gCurrent) {
            ...
            gCurrent.selected = !gCurrent.selected;
        } else { // check if we are over anything
            var row = gBox.row
            , col = gBox.col
            , oAtom
            ;
            for (var i = 0; i < gItems.length; i++) {
                oAtom = gItems[i];
                if (oAtom.row === row && oAtom.col === col) {
                    set_current(oAtom);
                    break;
                }
            }
    ...
    function move_box(dir) {
        ...
        switch(dir) {
        case 'left': // find the next oAtom to the left of this one
            comparator = function(atom, box) {
                return atom.col < box.col;
            };
            break;
        case 'right':
            comparator = function(atom, box) {
        ...
        if (comparator) {
            foreach(gItems, function(oAtom) {
                var compare = comparator(oAtom, gBox);
                console.log('comparator returned', compare);
                if (compare) {
                    var d = calc_distance(row, col, oAtom.row, oAtom.col);
                    console.log('current', row, col, 'atom', oAtom.row, oAtom.col, d);
        ...
        if (closestAtom) {
            ...
            animationTime = gAnimationTime * Math.abs(gBox.row - closestAtom.row + gBox.col - closestAtom.col) / 2;
            xAniLine(gBox.box,
                gA.xpos(closestAtom.col),
                gA.ypos(closestAtom.row), 
                animationTime, 1, 
                function () {
                    gCurrent = closestAtom;
                    gCurrent.selected = false;
                    gBox.row = closestAtom.row;
                    gBox.col = closestAtom.col;
                }
            );
        }
    }
